...
# Function to approximate Newton's method
def approximate_newton(r_in, r_out, u_in=None, alph=0.1, l2=1e-4):
...
# Function to solve CoRNN using ADMM on CPU
def solve_corrn_admm(r_in, r_out, u_in=None, alph=0.1, l2=1e-4, threshold=1, rho=100,
                     verbose=0, check_convergence=0, mask=None, num_iters=30, gnd=None, solver_type='weighted'):
    if verbose:
...
# Function to solve CoRNN using ADMM on GPU
def solve_corrn_admm_gpu(r_in, r_out, u_in=None, alph=0.1, l2=1e-4, threshold=1, rho=100,
                         verbose=0, mask=None, float_type='32bit', num_iters=30, gnd=None, solver_type='weighted'):
    if verbose:
        temp = time_now.localtime()
        current_time = time_now.strftime("%H:%M:%S", temp)
        print('%s: Running CoRNN solver.' % (current_time))
    ...
    reg_term = torch.tensor((l2 + rho) * np.diag(np.ones(n_tot)), device=device, dtype=dtype)
    ...
    for idx in range(num_iters):
        ...
        if solver_type == 'weighted':
            ...
            E_pred = (d - dhat) / (1 - d**2)
            per_not_used = 0
            E_pred[abs(E_pred) > threshold] = threshold * torch.sign(E_pred[abs(E_pred) > threshold])

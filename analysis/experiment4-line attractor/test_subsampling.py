...
n_rec = 500  # Number of recurrent units
T = 1000  # Sequence length
width = 20  # Width of the Gaussian
gaus_int = 500  # Center of the Gaussian

# Define subsampling parameters
n_sup_all = np.logspace(2, np.log10(5000), 8).astype(int) + 1  # Number of support units
num_exps = 1  # Number of experiments
num_trials = np.array([30, 50, 100]).astype(int)  # Number of trials

# Initialize result arrays
cor_weights_rec = np.zeros([num_exps, num_trials.shape[0], n_sup_all.shape[0]])  # Correlation of recurrent weights
cor_weights_in = np.zeros([num_exps, num_trials.shape[0], n_sup_all.shape[0]])  # Correlation of input weights
r2_units = np.zeros([num_exps, num_trials.shape[0], n_sup_all.shape[0]])  # R2 score for units
r2_units_dist = np.zeros([num_exps, num_trials.shape[0], n_sup_all.shape[0]])  # R2 score for units with disturbance
times = np.zeros([num_exps, num_trials.shape[0], n_sup_all.shape[0]])  # Time taken for each experiment
alph = 0.1  # Alpha value for the model

# Run experiments
for kk in range(num_exps):
    r_in_all = np.zeros([T*100, n_rec])  # Input recurrent states
    r_out_all = np.zeros([T*100, n_rec])  # Output recurrent states
    u_all = np.zeros([T*100, n_in])  # Input data
    
    model = Model(n_in, n_rec, n_in)
    model = torch.load('models/model_%d.pt' % np.mod(kk, 10), weights_only=False)
    model.to('cpu')
    model.alpha = 0.1
    ...
    for ll in range(num_trials.shape[0]):
        ...
        for ss in range(n_sup_all.shape[0]):
            ...
            print('%s: Experiment %d. Num Trials %d. N_sup %d. Time: %.2f. Input %.2f. Rec %.2f. R2 %.2f and %.2f.' % (
                current_time, kk, n_trial, n_sup, times[kk, ll, ss], cor_weights_in[kk, ll, ss],
                cor_weights_rec[kk, ll, ss], r2_units[kk, ll, ss], r2_units_dist[kk, ll, ss]))
